...
vec3 sampleCosineWeightedDirection(vec2 uu, BrdfData brdf) {
  vec3 normal = brdf.N;
  float up = sqrt(uu.x);
...
vec3 sampleDirectionGGX(vec2 u, BrdfData brdf) {
  vec3 H;
  if (brdf.alpha == 0.0) {
    H = vec3(0.0, 0.0, 1.0);
  } else {
...
vec3 evalGGXVNDF(BrdfData brdf, vec3 L) {
  vec3 H = normalize(brdf.V + L);
  float HoL = clamp(dot(H, L), 0.00001, 1.0);
...
float samplePdfDistribute1D(float u, float[3] func, out uint index) {
  int n = func.length();
  float[4] cdf;
  cdf[0] = 0.0;
  for (int i = 0; i < n; i++) {
...
void evalBsdfWeight(Prd prd, Material material, vec3 viewDirection,
  ...
  if (dot(-viewDirection, outDirection) > 0.9999) {
    ...
    vec3 specularWeight = evalGGXVNDF(brdfData, L);
...
float evalBsdfPdf(Prd prd, Material material, vec3 viewDirection,
                  vec3 outDirection) {
  MaterialData materialData = getMaterialData(prd, material, viewDirection);
  BrdfData brdfData = getBrdfData(prd, materialData, viewDirection);
...
bool sampleBsdf(float[3] u, Prd prd, Material material, vec3 viewDirection,
                out vec3 outDirection, out vec3 bsdfWeight, out vec3 emissive) {
  MaterialData materialData = getMaterialData(prd, material, viewDirection);
  BrdfData brdfData = getBrdfData(prd, materialData, viewDirection);
  ...
  case 0: {
    ...
    vec3 L = sampleDirectionGGX(uu, brdfData);
  ...
  case 1: {
    // diffuse
    vec2 uu = vec2(u[1], u[2]);
    vec3 L = sampleCosineWeightedDirection(uu, brdfData);
