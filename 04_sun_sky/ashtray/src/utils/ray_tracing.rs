...
pub struct HitShaderModules {
    ...
    pub closest_hit: Option<crate::ShaderModuleHandle>,
...
pub fn create_ray_tracing_pipelines(
    ...
    let hit_shader_stages = hit_shader_modules
        .iter()
        .flat_map(|module| {
            let closest_hit = module.closest_hit.as_ref().map(|module| {
                vk::PipelineShaderStageCreateInfo::builder()
    ...
    for hit_shader_module in hit_shader_modules {
        let closest_hit_shader_index = if hit_shader_module.closest_hit.is_some() {
            index += 1;
            index - 1
        } else {
